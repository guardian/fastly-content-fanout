...
interface EventbridgeToFanoutStackProps extends GuStackProps {
	...
	snsTopicDetail: {
		...
		pathExtractionPattern: string;
		maybeFilterObject?: object;
	};
}
...
export class EventbridgeToFanout extends GuStack {
	constructor(scope: App, id: string, props: EventbridgeToFanoutStackProps) {
		...
		const snsTopic = sns.Topic.fromTopicArn(
			this,
			'SNSTopic',
			Fn.importValue(props.snsTopicDetail.cfnExportName),
		);
		...
		new pipes.CfnPipe(this, 'PipeFromSqsToEventBridge', {
			name: `pipe-from-${sqsQueue.queueName}-to-${eventBridgeBus.eventBusName}`,
			roleArn: putEventsOnPipeRole.roleArn,
			source: sqsQueue.queueArn,
			sourceParameters: props.snsTopicDetail.maybeFilterObject && {
				filterCriteria: {
					filters: [
						{
							pattern: JSON.stringify(props.snsTopicDetail.maybeFilterObject),
						},
					],
				},
			},
			...
			targetParameters: {
				inputTemplate: JSON.stringify({
					path: props.snsTopicDetail.pathExtractionPattern,
				}),
			},
		});
	}
